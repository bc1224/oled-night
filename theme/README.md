...
A companion Chrome theme that makes the browser window itself true black: the tab strip, toolbar, address bar, bookmarks bar and new-tab page. The OLED Night extension recolors websites, but no extension can change Chrome's own window. A theme can, so the two are made to be used together.
