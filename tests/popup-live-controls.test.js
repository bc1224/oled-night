...
setImmediate(async () => {
  ...
  assert.deepEqual(messages.at(-1), { id: 7, message: { type: "oled-night-preview", patch: { brightness: 73 } } });
  element("brightness").listeners.change({ target: { value: "73" } });
  await new Promise((resolve) => setTimeout(resolve, 10));
  assert.deepEqual(writes.at(-1), { brightness: 73 });
  console.log("popup-live-controls: 3 assertions passed");
});
